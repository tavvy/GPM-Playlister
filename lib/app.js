...
// can throw Err
var schema = JSON.parse(JSON.stringify(require(path.join(__dirname, '..', 'config/schema.json'))));
var presetStations = JSON.parse(JSON.stringify(require(path.join(__dirname, '..', 'config/stations.json'))));
...
function app(options) {
	// set the schema to Playlister or Station
		...
		async.waterfall([
				...
				fetchPlrTracklist(options, function(err, plrTracklist) {
					if (!err) {
						reporter.info(
							...
							'\n\nSearching GPM for matching tracks...'
						);
					}
			...
			function(plrTracklist, cbAsync) {
				...
				fetchGPMStoreIds(plrTracklist.track_list, options, function(err, matchedTracklist) {
					if (!err) {
						reporter.info('Finished searching GPM, matched ' + matchedTracklist.matches + ' of ' + matchedTracklist.track_list.length + ' tracks');
			...
			function(plrTracklist, cbAsync) {
				...
				reporter.info('\nPushing playlist to Goolge Play Music...');
 ...
 * @param {Object} - loginInfo - userConfig loaded from config/user-data.json
 * @return {Boolean} - isValid - if user login details are saved
 */
function checkLogin(loginInfo) {
	var isValid = false;
	if (loginInfo.androidId && loginInfo.masterToken) {
		isValid = true;
	}
	return isValid;
}
