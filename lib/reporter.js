...
Object.keys(theme).forEach(level => {
	exports[level] = (message) => console.log(theme[level](message));
});
/*
 * REPORT MATCHES
 ...
 */
exports.match = (matchType, query, match) => {
	let matchMessage;

	if (matchType && match) {
		matchMessage = matchType === 3 ? 'User match ' : 'Found match';
	...
	matchMessage = matchType === 0 ? 'No match   ' : 'No results ';
